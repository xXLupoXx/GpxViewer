...
namespace GPX_Viewer
{
    public class Import
    {
        enum GPX_Zustände { START, GPX, GPX_TRK, GPX_WPT, GPX_TRK_NAME, GPX_TRK_TRKSEG, GPX_TRK_TRKSEG_TRKPT, GPX_TRK_TRKSEG_TRKPT_TIME, GPX_TRK_TRKSEG_TRKPT_ELE };
        GPX_Zustände Zustand = GPX_Zustände.START;
        double akt_lat, akt_lon;
        Model m;
        int Anzahl_der_Tracks, ID_Track_var, GPX_Index_Counter, debug = 0, Max_Index, Min_Index;
        string SQL;
        string Waypoint_Name = null;
        string Track_Name = null;
        string akt_Zeit, strBuf;
        double akt_ele;
        ...
        public void ImportGPX(FolderBrowserDialog Ordner_Browser)
        {
            ...
            foreach (FileInfo f in d.GetFiles("*.gpx"))
            {
                ...
                XmlTextReader xr = new XmlTextReader(Path.Combine(Ordner_Browser.SelectedPath, f.Name));
                #region Reader
                while (xr.Read())
                {
